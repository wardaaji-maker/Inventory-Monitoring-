const CONFIG = {
  ...
  // Global sheets shared across all events
  globalSheets: {
    settings: 'Settings',
    templates: 'MessageTemplates',
    events: 'Events'
  },
  // Sheets that are specific to each event
  eventSheets: {
    leads: 'Leads',
...
function getEvents() {
  try {
    ...
    if (data.length === 0 || (data.length > 0 && data[0][0] !== 'EventName')) {
        const headers = ['EventName', 'EventID', 'CreatedAt', 'Status'];
        sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
        data = sheet.getDataRange().getValues();
    }

    // If only headers exist, add Main Event
    if (data.length <= 1) {
        const defaultEvent = ['Main Event', '', new Date(), 'Active'];
        sheet.appendRow(defaultEvent);
        return [{name: 'Main Event', id: ''}];
    }
    ...
    Logger.log('Error in getEvents: ' + e.toString());
    return [{name: 'Main Event', id: ''}]; // Fallback
  }
}
...
function createEvent(eventName) {
  try {
    ...
    if (existingEvents.some(e => e.name.toLowerCase() === eventName.toLowerCase())) {
      return { success: false, error: "Event name already exists" };
    }

    // Generate simple ID (suffix)
    const eventId = '_' + eventName.replace(/[^a-zA-Z0-9]/g, '');

    // Add to Events sheet
    sheet.appendRow([eventName, eventId, new Date(), 'Active']);
    ...
    Object.keys(CONFIG.eventSheets).forEach(key => {
      ...
      const newSheetName = baseName + eventId;

      let newSheet = ss.getSheetByName(newSheetName);
      if (!newSheet) {
        newSheet = ss.insertSheet(newSheetName);

        const headers = getSheetHeaders(baseName); // Use base name to get headers
        if (headers.length > 0) {
...
function getSheet(name, eventId = '') {
  try {
    ...
    // Resolve sheet name
    let baseName = CONFIG.globalSheets[name];
    let isEventSheet = false;

    if (!baseName) {
      baseName = CONFIG.eventSheets[name];
      isEventSheet = true;
    }

    if (!baseName) {
       throw new Error(`Sheet configuration for '${name}' not found.`);
    }

    let finalSheetName = baseName;
    if (isEventSheet && eventId) {
      finalSheetName = baseName + eventId;
    }
    ...
    if (!sheet) {
      Logger.log(`Sheet '${finalSheetName}' not found. Creating it now...`);
      sheet = ss.insertSheet(finalSheetName);

      // Set up headers for the new sheet
      // We pass baseName to setupSheetHeaders to ensure it gets the right columns
      setupSheetHeaders(baseName, sheet);
      Logger.log(`Created and initialized sheet: ${finalSheetName}`);
    }

    return sheet;

  } catch (e) {
    Logger.log(`Error in getSheet('${name}', '${eventId}'): ${e.toString()}`);
    throw e;
  }
}
...
// Set up all required sheets (Defaults/Main Event)
function setupSheets() {
  try {
    const ss = SpreadsheetApp.openById(CONFIG.sheetId);

    // Setup Global Sheets
    Object.keys(CONFIG.globalSheets).forEach(key => {
      const sheetName = CONFIG.globalSheets[key];
      let sheet = ss.getSheetByName(sheetName);
      if (!sheet) {
        sheet = ss.insertSheet(sheetName);
        Logger.log(`Created global sheet: ${sheetName}`);
      }
      setupSheetHeaders(sheetName, sheet);
    });

    // Setup Default Event Sheets
    Object.keys(CONFIG.eventSheets).forEach(key => {
      const sheetName = CONFIG.eventSheets[key];
      let sheet = ss.getSheetByName(sheetName);

      if (!sheet) {
        sheet = ss.insertSheet(sheetName);
        Logger.log(`Created event sheet: ${sheetName}`);
      }
...
function getSheetHeaders(sheetName) {
  switch(sheetName) {
    ...
    case 'Events':
      return ['EventName', 'EventID', 'CreatedAt', 'Status'];
    default:
      return [];
  }
}
...
function forceUpdateHeaders() {
  ...
  if (response == ui.Button.YES) {
    try {
      ...
      // Update Event Sheets (All Active Events)
      ...
      events.forEach(event => {
          ...
          Object.keys(CONFIG.eventSheets).forEach(key => {
            const baseName = CONFIG.eventSheets[key];
            const sheetName = suffix ? baseName + suffix : baseName;

            const headers = getSheetHeaders(baseName);
...
function getActiveLeads(eventId) {
  const allLeads = getLeads(eventId);
...
function getLeads(eventId) {
  try {
    const sheet = getSheet('leads', eventId);
...
function addLead(leadData, eventId) {
  try {
    ...
    // Add to history
    addFollowUpHistory(leadId, now, 'Initial Contact', 'Completed', 'Lead created', 'System', eventId);
...
function updateLeadNote(leadId, note, eventId) {
  try {
    if (!leadId || !note) {
...
function addFollowUpHistory(leadId, date, type, status, notes, user, eventId) {
  try {
    const sheet = getSheet('followups', eventId);
...
function updateStatus(leadId, newStatus, dealValue = '', receiptNumber = '', lostReason = '', eventId) {
  try {
    if (!leadId || !newStatus) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (String(data[i][leadIDColIndex]) === String(leadId)) {
        ...
        addFollowUpHistory(leadId, new Date(), 'Status Change', newStatus, historyNotes, getCurrentUser(), eventId);
...
function updateLeadDetails(payload) {
    const { leadId, newStatus, dealValue, receiptNumber, reasonForLoss, eventId } = payload;
    return updateStatus(leadId, newStatus, dealValue, receiptNumber, reasonForLoss, eventId);
}

function updateDealValue(leadId, dealValue, eventId) {
  try {
    const sheet = getSheet('leads', eventId);
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][leadIDColIndex] === leadId) {
        ...
        addFollowUpHistory(leadId, new Date(), 'Deal Value Update', 'Completed',
                          `Deal value set to: ${dealValue}`, getCurrentUser(), eventId);
...
function getTemplateVariables(leadId, eventId) {
  try {
    let variables = {
      name: 'Customer',
      product: 'our product',
    ...
    // Get lead data
    if (leadId) {
      const leads = getLeads(eventId); // Re-use existing getLeads
...
function recordFollowUp(leadId, followUpData, eventId) {
  try {
    ...
    const leadsSheet = getSheet('leads', eventId);
    ...
    addFollowUpHistory(leadId, new Date(), followUpData.type, 'Completed', fullNote, getCurrentUser(), eventId);

    // --- NEW: Update the main lead note ---
    if (followUpData.notes) { // Only update if there are internal notes
      updateLeadNote(leadId, followUpData.notes, eventId);
    }
...
function markAsContacted(leadId, contactType, notes, eventId) {
  try {
    ...
    // Add history entry
    addFollowUpHistory(leadId, new Date(), contactType, 'Completed', notes, getCurrentUser(), eventId);

    // --- NEW: Update the main lead note ---
    updateLeadNote(leadId, notes, eventId);
...
function checkFollowUps() {
  try {
    ...
    events.forEach(event => {
      const leads = getLeads(event.id); // Get all leads for event
      const today = new Date();
      today.setHours(0, 0, 0, 0);
...
function recordQuotation(leadId, eventId) {
  try {
    if (!leadId) {
    ...
    addFollowUpHistory(leadId, new Date(), 'Quotation Created', 'Completed', 'Quotation created via system', getCurrentUser(), eventId);
...
function dailyUpdate() {
  ...
  try {
    Logger.log('Starting daily update with backfill logic for all events...');
    ...
    if (lastRunDateStr) {
      ...
      // On the very first run, start from when the first lead was created (globally or from Main).
    ...
    while (currentDate < today) {
      ...
      events.forEach(event => {
         ...
         const allLeads = getLeads(eventId);
         const allFollowUps = getSheet('followups', eventId).getDataRange().getValues();

         updateMonthlyLeadSummaryForDate(dateToProcess, allLeads, allFollowUps, eventId);
         updateDailyDashboardStatsForDate(dateToProcess, allLeads, allFollowUps, eventId);
      });
 ...
 */
function getFirstLeadDate(eventId) {
    const leadsSheet = getSheet('leads', eventId);
 ...
 */
function updateMonthlyLeadSummaryForDate(dateToProcess, allLeadsData, allFollowUpsData, eventId) {
  const summarySheet = getSheet('monthlyLeadSummary', eventId);
 ...
 */
function updateDailyDashboardStatsForDate(dateToProcess, allLeadsData, allFollowUpsData, eventId) {
    const statsSheet = getSheet('dailyDashboardStats', eventId);
 ...
 */
function updateTeamPerformanceDashboard(eventId) {
    const ui = SpreadsheetApp.getUi();

    try {
        const events = eventId ? [{id: eventId}] : getEvents();
        let updatedCount = 0;

        events.forEach(event => {
            if (updateTeamPerformanceDashboardForEvent(event.id)) {
              updatedCount++;
            }
        });

        // Only show alert if UI is available (manual run)
        if (ui) {
             ui.alert(`Team Performance Dashboard updated for ${updatedCount} events.`);
        }
        ...
        if (ui) ui.alert(`An error occurred while updating the dashboard: ${e.message}`);
    }
}

function updateTeamPerformanceDashboardForEvent(eventId) {
    try {
        const allLeads = getLeads(eventId);
        ...
        const allFollowUps = getSheet('followups', eventId).getDataRange().getValues().slice(1);
        ...
        const sheet = getSheet('teamPerformanceDashboard', eventId);
        if (sheet.getLastRow() > 1) {
        ...
        Logger.log(`Error updating Team Performance Dashboard for event ${eventId}: ${e.toString()}`);
        return false;
    }
}
...
function syncAllLeadNotes() {
  ...
  try {
    ...
    events.forEach(event => {
        Logger.log('Starting syncAllLeadNotes for event: ' + event.name);
        const leadsSheet = getSheet('leads', event.id);
        const followupsSheet = getSheet('followups', event.id);
        ...
        if (leadIdCol === -1 || notesCol === -1) {
          Logger.log('Could not find LeadID or Notes column in Leads sheet for event ' + event.name);
          return;
        }
    ...
    Logger.log(`Sync complete. Updated ${totalUpdated} lead notes across all events.`);
    ui.alert(`Sync Complete!`, `Successfully updated the notes for ${totalUpdated} leads based on their follow-up history.`, ui.ButtonSet.OK);
