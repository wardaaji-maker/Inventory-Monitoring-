...
function getActiveLeads() {
  ...
  return allLeads.filter(lead => {
      ...
      //    - If Status is 'Won', 'Lost', HIDE it (Historical closed leads).
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (String(data[i][leadIDColIndex]) === String(leadId)) {
        ...
        // If won or lost, clear next follow-up (check if column exists)
...
function getTemplateVariables(leadId) {
  try {
    ...
    return { name: 'Customer', product: 'our product', business: 'Your Business Name', phone: '', domicile: '', leadType: '' }; // Return safe defaults
  }
}
